...
def create_visualization(traj_data):
    ...
    ax3.plot(traj_data['time'], vel_mag, 'k-', linewidth=2, label='Magnitude')
    ax3.plot(traj_data['time'], traj_data['vel_x'], 'r-', label='Vel X')
    ax3.plot(traj_data['time'], traj_data['vel_y'], 'g-', label='Vel Y')
    ax3.plot(traj_data['time'], traj_data['vel_z'], 'b-', label='Vel Z')
    # ax3.axhline(y=15.0, color='r', linestyle='--', alpha=0.7, label='V Limit')
    ax3.set_title('Velocity Components & Magnitude')
    ax3.set_xlabel('Time (s)')
    ax3.set_ylabel('Velocity (m/s)')
    ax3.legend()
    ax3.grid(True)
    
    # Acceleration magnitude and components
    ax4 = fig.add_subplot(2, 2, 4)
    acc_mag = np.sqrt(traj_data['acc_x']**2 + traj_data['acc_y']**2 + traj_data['acc_z']**2)
    ax4.plot(traj_data['time'], acc_mag, 'k-', linewidth=2, label='Magnitude')
    ax4.plot(traj_data['time'], traj_data['acc_x'], 'r-', label='Acc X')
    ax4.plot(traj_data['time'], traj_data['acc_y'], 'g-', label='Acc Y')
    ax4.plot(traj_data['time'], traj_data['acc_z'], 'b-', label='Acc Z')
    # ax4.axhline(y=12.0, color='r', linestyle='--', alpha=0.7, label='A Limit')
    ax4.set_title('Acceleration Components & Magnitude')
